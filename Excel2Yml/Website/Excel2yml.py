...
def transition(xlsx_file, vip=None, domain_name=None):
  ...
  if vip is not None and domain_name is None:
    domain_name = vip
  if vip is not None:
    for service in new_yml_obj:
        service_name = string_convert(service.get('service'))
        if service_name == "nginxha":
            service['configList']['keepalived_common']['default_virtual_ip_address'] = vip
  ...
  with open('D:\\CloudMusic\\xwphs.yml', 'w', encoding='utf8') as f:
      yaml.dump(new_yml_obj, f)
      f.flush()
  with open('D:\\CloudMusic\\xwphs.yml', 'r', encoding='utf8') as f:
      str = f.read()
      return str
